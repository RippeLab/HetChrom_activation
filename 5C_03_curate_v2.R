...
dapi_overlays <-list.files(input_folder)[grep("dapi_with_masks",list.files(input_folder))]
dcas9_overlays <-list.files(input_folder)[grep("dcas9_with_masks",list.files(input_folder))]

# check if segmentCC run was complete
if (length(dapi_overlays)!=length(rownames(positions))){print("WARNING: It seems that not all of your images have been segmented. This can happen if a run of segmentCC was aborted. Please check if every image has a corresponding _dapi_with_masks.tif processed file.")}
...
mean_sd_projection_seg <- mean_sd_projection[which(mean_sd_projection$mean_dapi_cc!=0 & mean_sd_projection$area!=40000),]
...
for (i in 1:length(rownames(positions))){
  dispIm1 <- readImage(file=paste0(input_folder,dapi_overlays[i]))
...
colnames(mean_sd_projection_curated)[37] <- "annotation"

# write results into mean_sd_projections_curated.csv
dir.create(paste0(input_folder,timestamp,"_curateCC_results"))
write.table(file=paste0(input_folder,timestamp,"_curateCC_results","/","mean_sd_projection_curated.csv"), mean_sd_projection_curated, sep = "\t", append = F, row.names = F, quote = F)
