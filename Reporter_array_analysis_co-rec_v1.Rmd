...
reporterdir <- 'insert file path/VP16/ms2_fish/'
...
lapply(imagenames, function(filename){
  ...
  reporter <- readImage(paste0(reporterdir, '561nm_corr_', filename, '.tif'))
...
halo_nuc_thres <- 0.002309659
gfp_nuc_thres <- 0.001557401
...
silence_threshold_RNase_mean <-  0.012219989
...
combined_percent_VP16 <- data_frame(percentage_active_Ctrl, percentage_active_mHP1alpha, percentage_active_KRAB, percentage_silent_Ctrl, percentage_silent_mHP1alpha, percentage_silent_KRAB)
...
write.csv(combined_no_dox, file = paste0(outdir, timestamp, "_data", "/reporter_signal_nodox_vp16.csv"), row.names = FALSE)
write.csv(combined_percent_VP16, file = paste0(outdir, timestamp, "_data", "/combined_percent_active-silent_vp16.csv"), row.names = FALSE)
write.csv(combined_active, file = paste0(outdir, timestamp, "_data", "/reporter_signal_vp16.csv"), row.names = FALSE)
