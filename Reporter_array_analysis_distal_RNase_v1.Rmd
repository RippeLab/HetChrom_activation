...
summary_reporter_nuc <- all_filt %>%
  summarize(
    ...
    ) %>% mutate(upper_whisker_reporter_persistent = (Q3 + 1.5 * IQR))
write.csv(summary_reporter_nuc, file = paste0(outdir, timestamp, "_data", "/bckg_reporter_persistent.csv"), row.names = FALSE)
```
